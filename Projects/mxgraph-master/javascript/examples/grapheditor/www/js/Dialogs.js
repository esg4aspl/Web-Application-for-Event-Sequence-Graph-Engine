...
var FilenameDialog = function(editorUi, filename, buttonText, fn, label, validateFn, content, helpLink, closeOnBtn, cancelFn)
{
	...
	var genericBtn = mxUtils.button(buttonText, function()
	{
	  ...
	  //save again esg
	editorUi.saveAsJSON(nameInput.value);
...
/**
 * Construct test case dialog
 */
var TestCaseDialog= function(editorUi){
	var div = document.createElement('div');
	...
	//	var ESG = { dateTime : dateTime, name :  editorUi.sidebar.getESGName(),xmlVersion:xmlString,vertices:[],edges:[]};
	    var ESG = { dateTime : dateTime, name :  editorUi.sidebar.getESGName(),featureName : editorUi.sidebar.getFESGName(), xmlVersion:xmlString,vertices:[],edges:[]};
	    ...
	    var url =  'http://localhost:9000/generateTestCases';
	 ...
	 //  xhr.setRequestHeader("Content-Type", "application/text");
	    ...
	    if(xhr.readyState === 4 && xhr.status === 200){
	     ...
	     /*   var testcases = xhr.responseText;
	       var parsedTestCases =  testcases.split("*");
	       
		    	    for(var i = 0;i<parsedTestCases.length;i++){
		    	     esgListLi= document.createElement('li');
		    	  
		    	    var esgListText = document.createTextNode(parsedTestCases[i]);
		    	  
		    	    esgListLi.appendChild(esgListText);
		    	    esgListUl.appendChild(esgListLi);
		    	    }
		    	   div.appendChild(esgListUl);*/
    	//  var textNode = document.createTextNode(testcases);
       //    div.appendChild(xhr.responseText);
         //  console.log(xhr.responseText);
           var newElem = document.createElement("BR");
         var downloadBtn = mxUtils.button(mxResources.get('download'), function()
	      {
	       var data =xhr.responseText,fileName = "testcases.txt";
	      ...
	      div.insertBefore(downloadBtn, div.childNodes[0]);
	      div.insertBefore(newElem, div.childNodes[1]);
    	  }
       };
       xhr.send(fileJSON);

...
Sidebar.prototype.addESGNamePalette = function(expand){
	...
	esgnameText.setAttribute('placeholder', 'Enter cesg or pure name');
    inner.appendChild(esgnameText);
	

	var cross = document.createElement('img');
//	cross.setAttribute('src', Sidebar.prototype.nextLargeImage);
	cross.setAttribute('class',"geSprite geSprite-plus");
	cross.style.position = 'relative';
	cross.style.left = '-18px';
	cross.style.bottom = '-7px';
	cross.style.top = '5px';
	...
	//f esg input
	var esgdiv = document.createElement('div');
	esgdiv.style.whiteSpace = 'nowrap';
	esgdiv.style.textOverflow = 'clip';
	esgdiv.style.paddingBottom = '8px';
	esgdiv.style.cursor = 'default';
    
	var fesgnameText = document.createElement("input");
	fesgnameText.setAttribute('type', 'text');
	fesgnameText.setAttribute('placeholder', 'Enter fesg name');
	esgdiv.appendChild(fesgnameText);
	

	var img = document.createElement('img');
	//img.setAttribute('src', Sidebar.prototype.addNameImage);
	img.setAttribute('class',"geSprite geSprite-plus");
	img.style.position = 'relative';
	img.style.left = '-18px';
	img.style.bottom = '-7px';
	img.style.top = '5px';
    // Workaround for blocked text selection in Editor
    mxEvent.addListener(fesgnameText, 'mousedown', function(evt)
    {
    	if (evt.stopPropagation)
    	{
    		evt.stopPropagation();
    ...
    esgdiv.appendChild(img);
    div.appendChild(esgdiv);
    ...
    // Keeps references to the DOM nodes
	this.palettes['fesgname'] = [elt, outer];
	mxEvent.addListener(img, 'click', function()
			{
		    fesgname = fesgnameText.value;
		    console.log(fesgname);
		    });
...
Sidebar.prototype.getFESGName = function(){
	return fesgname;
}

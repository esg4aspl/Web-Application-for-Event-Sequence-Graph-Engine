		...
		if (window.opener != null && window.opener.openFile != null)
		{
			...
			window.opener.openFile.setConsumer(mxUtils.bind(this, function(xml, filename)
			{
				try
				{
					...
					console.log(this.editor.setGraphXml(doc.documentElement));
...
/*	this.showDialog(new OpenDialog(this).container, (Editor.useLocalStorage) ? 640 : 320,
	...
	this.showDialog(new OpenDialog(this).container,500, 500, true, true, function()
	{
		window.openFile = null;
	});
};
...
EditorUi.prototype.sendJSON = function(){ 
	...
	var graphEdges =[];
	   ...
	   for( var i = 2 ; i < cellSize ; i++){
		...
		console.log(this.sidebar.getFESGName());
		var ESG = { dateTime : dateTime, name :  this.sidebar.getESGName(),featureName : this.sidebar.getFESGName(), xmlVersion:xmlString,vertices:[],edges:[]};
	    ...
	    var url =  'http://localhost:9000/saveESG';
...
//generate test cases
EditorUi.prototype.generateTestCases = function(){
	var graph = this.editor.graph;
	var model = graph.getModel();
    var cell = model.getCell(1);
    var root = model.getRoot(cell);
...
//saveAsJSON
EditorUi.prototype.saveAsJSON=function(filename){
	var graph = this.editor.graph;
	var model = graph.getModel();
    var cell = model.getCell(1);
    var root = model.getRoot(cell);
	...
	var graphEdges =[];
	   ...
	   for( var i = 2 ; i < cellSize ; i++){
		...
		var ESG = { dateTime : dateTime, name :  filename,xmlVersion:xmlString,vertices:[],edges:[]};
     ...
     //   console.log(fileJSON);
        var xhr = new XMLHttpRequest();   // new HttpRequest instance 
	    var url =  'http://localhost:9000/saveAsESG';
	    xhr.open('POST',url, true);

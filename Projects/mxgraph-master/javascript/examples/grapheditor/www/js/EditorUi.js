...
/*	this.showDialog(new OpenDialog(this).container, (Editor.useLocalStorage) ? 640 : 320,
	...
	this.showDialog(new OpenDialog(this).container,300, 220, true, true, function()
	{
		window.openFile = null;
	});
};
...
EditorUi.prototype.sendJSON = function(){ 
    ...
    var graphEdges =[];
	   ...
	   console.log(graphVertices.length);
		...
		var ESG = { ID : dateTime, name : this.sidebar.getESGName(), vertices:[], edges:[]};
...
//saveAsJSON
EditorUi.prototype.saveAsJSON=function(){
	console.log("ajsgfajfs");
	var graph = this.editor.graph;
	var model = graph.getModel();
    var cell = model.getCell(1);
    var root = model.getRoot(cell);
       ...
       jsonhttp.send(JSON.stringify(ESG));*/
	    ...
	    xhr.onreadystatechange = function () {
	    	...
	    	//    for(i = 0;i<infos.length;i++){
	    	    //	var esgListIl = document.createElement('li');
	    	    //	var esgListText = document.createTextNode(infos[i].name);
	    	    //	esgListIl.appendChild(esgListText);
	    	   
	    	    //	esgListUl.appendChild(esgListIl);
	    	  //  }

...
mxDoubleEllipse.prototype.paintForeground = function(c, x, y, w, h)
{
	if (!this.outline)
	{
		var margin = mxUtils.getValue(this.style, mxConstants.STYLE_MARGIN, Math.max(3 + this.strokewidth, Math.min(w / 5, h / 5)));
		x += margin;
		y += margin;
		w -=( 2 * margin) ;
		h -=( 2 * margin) ;
...
mxDoubleEllipse.prototype.getLabelBounds = function(rect)
{
	...
	return new mxRectangle(rect.x + margin, rect.y + margin, rect.width - 3 * margin, rect.height - 3 * margin);
};

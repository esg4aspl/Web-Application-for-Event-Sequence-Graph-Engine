...
package controllers.javascript {
  ...
  class ReverseESGController(_prefix: => String) {
    ...
    // @LINE:24
    def generateTestCases: JavaScriptReverseRoute = JavaScriptReverseRoute(
      "controllers.ESGController.generateTestCases",
      """
        function() {
          return _wA({method:"POST", url:"""" + _prefix + { _defaultPrefix } + """" + "generateTestCases"})
        }
      """
    )
